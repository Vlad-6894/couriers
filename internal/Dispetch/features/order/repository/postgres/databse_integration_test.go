...
func TestDatabase_integration(t *testing.T) {
	...
	dispetchDatabase := NewDispetchRepositoryPostgres(pool)

	t.Run("test dispetch orders feature postgres", func(t *testing.T) {

		info, err := dispetchDatabase.GetFreeCouriers(ctx)
		if err != nil {
			t.Errorf("fail to get free couriers: %v", err)
		}

		if len(info) == 0 {
			t.Errorf("fail get info: info size must not be 0")
		}

		id, version, err := dispetchDatabase.SearchCourier(ctx, "Moscow")
		if err != nil {
			t.Errorf("fail to search courier: %v", err)
		}

		if err := dispetchDatabase.DoBusy(ctx, id, version); err != nil {
			t.Errorf("fail do busy courier: %v", err)
		}
	})
}
