...
func TestLoginCourier(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			waitErr:  nil,
			prepareMock: func(m *auth_mocks.MockAuthDatabaseRepository) {
				m.EXPECT().GetCourier(gomock.Any(), gomock.Any()).Return(auth_domains.Courier{
					ID:       1,
					Version:  1,
					Login:    "123456789",
					Password: "123456789",
...

		{
			...
			name:     "Fail Login courier because password is wrong",
			login:    "123456789",
			password: "987654321",
			wantErr:  true,
...

		{
			...
			waitErr:  errDatabase,
			prepareMock: func(m *auth_mocks.MockAuthDatabaseRepository) {
				m.EXPECT().GetCourier(gomock.Any(), gomock.Any()).Return(auth_domains.Courier{Password: "12345678"}, errDatabase).Times(1)
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			key, err := servise.LoginCourier(t.Context(), tt.login, tt.password)
