...
func TestDatabase_integration(t *testing.T) {
	...
	if _, err := pool.Exec(contextWithTimeIns, sqlRequestInsert, "123456789", "123456789", "Moscow"); err != nil {
		t.Fatalf("fail to insert user: %v", err)
	}

	ordersDatabase := NewDatabasePostgres(pool)

	t.Run("start Create Order", func(t *testing.T) {
		order := orders_domains.NewUnitializedOrder("cake", 1000, "Moscow", 1)

		newOrder, err := ordersDatabase.CreateOrder(ctx, order)
		if err != nil {
			t.Errorf("fail to create order: %v", err)
		}

		orders, err := ordersDatabase.GetOrders(ctx, newOrder.ID, nil, nil)
		if err != nil {
			t.Errorf("fail to get orders: %v", err)
		}

		if len(orders) != 1 {
			t.Errorf("orders too little!")
		}
	})
}
